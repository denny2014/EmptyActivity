 ...
 */
@ContentView(R.layout.activity_info_news_in_brief_info_detail)
public class NewsInBriefInfoDetailActivity extends BaseActivity implements DialogInterface.OnCancelListener {
...
//    private DocInfo info = null;
    private NewsInBrief mNewsInBrief=null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mNewsInBrief = (NewsInBrief) (savedInstanceState != null ? savedInstanceState.getSerializable("docInfo") : getIntent().getSerializableExtra("docInfo"));
        if (mNewsInBrief == null)
            finish();
        ...
        //fileId究竟是什么？？？？？
        boolean isCollect = SQLiteUtils.getInstance(mContext).isCollect(String.valueOf(mNewsInBrief.getFILE_ID()));
        tvCollect.setText(isCollect ? R.string.activity_detail_cancel_collect : R.string.activity_detail_collect);
        tvCollect.setTag(isCollect);
        Object object = SerializeUtils.readObject(new File(getCacheDir(), "user.info"));
        if (object != null) {
            UserInfo userInfo = (UserInfo) object;
            userId = userInfo.getUserId();
//            String url = Constants.SERVER_URL + "?" + ZetParams.getDetail(info.getFileId(), info.getFileNo(), info.getSetYear(), userId).toString();
//            LogUtils.e("----" + url);
//            HttpUtils.get(mContext, Constants.SERVER_URL, ZetParams.getDetail(info.getFileId(), mNewsInBrief.getFILE_NO(), mNewsInBrief.getSET_YEAR(), userId), HandlerFactory.getHandler(DetailHandler.class, new BaseNetHandler(new INetProxy(mContext, this, true), Constants.DETAIL_ACTION)));
            HttpUtils.get(mContext, Constants.SERVER_URL, ZetParams.getDetail("{20C181C3-EA21-4BD2-A9C9-2D5C34C4BB81}", "%E8%B4%A2%E7%BB%BC[2011]43%E5%8F%B7", "2014", userId), HandlerFactory.getHandler(DetailHandler.class, new BaseNetHandler(new INetProxy(mContext, this, true), Constants.DETAIL_ACTION)));
        } else {
            finish();
        }
    ...
    private void initWebView() {
		// TODO Auto-generated method stub
    	 //WebView加载web资源
    	String myUrl="http://121.43.231.19/api/show.php?action=getdescbyfileid&doctype=1&fileid={20C181C3-EA21-4BD2-A9C9-2D5C34C4BB81}&fileno=%E8%B4%A2%E7%BB%BC[2011]43%E5%8F%B7&setyear=2014&userid=1";
...
//                System.exit(0);//退出程序
	...
	private void AskTaskLoad(int tag) {
		...
		parameters.addParam("url", Constants.SERVER_URL_TEMP);
		parameters.addParam("action", "getdescbyfileid");
		parameters.addParam("doctype", "1");
		parameters.addParam("fileid", "{20C181C3-EA21-4BD2-A9C9-2D5C34C4BB81}");// fileNO1
		parameters.addParam("fileno", "%E8%B4%A2%E7%BB%BC[2011]43%E5%8F%B7");// sdocyear1
		parameters.addParam("setyear", "2014");
		parameters.addParam("userid", "1");//
    ...
    private void initView() {
        ...
        ((TextView) findViewById(R.id.tv_body_content)).setText(mNewsInBrief.getTITLE());
    }
    ...
    @OnClick({R.id.tv_back, R.id.tv_collect, R.id.tv_body_content})
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tv_collect:
                ...
                if (isCollect) {
//                    SQLiteUtils.getInstance(mContext).delCollect(mNewsInBrief.getFileId());
                } else {
//                    SQLiteUtils.getInstance(mContext).addCollect(mNewsInBrief);
                }
                isCollect = !isCollect;
                tvCollect.setText(isCollect ? R.string.activity_detail_cancel_collect : R.string.activity_detail_collect);
                tvCollect.setTag(isCollect);
...
//                lookFile(info.getFileName(), info.getFileSize());
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putSerializable("docInfo", mNewsInBrief);
    }
                ...
                for (DocInfo info : detailInfo.getRelative()) {
                    ...
                    textView.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            IntentUtils.launcher(mContext, NewsInBriefInfoDetailActivity.class, Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT, bundle);
                        }
                    });
                }
            } else {
    ...
    private void openPDF(Context context, File file) {
        ...
        if (versionName == null || (!localVersion.equals(versionName))) {
            if (versionName != null && (!localVersion.equals(versionName))) {
                DialogUtils.showConfirm(mContext, "温馨提示", "华为系统自动升级的WPS与政e通应用出现冲突。请卸载WPS后，即可正常使用政e通软件。", new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        PackageUtils.uninstallApk(mContext, PdfUtils.WPS_PACKAGE_NAME);
                    }
                });
            } else {
                if (!apkFile.exists()) {
